...
- QA 评估体系：50 题自然问法/人工关键词检索基线、真实生成基准、安全/边界检查和端到端 smoke。
- Agent 调度框架：工具化、路由、策略、受控补检索、trace、流式 API 和 baseline vs agent 对比评测。
- 内测部署：nginx 静态前端、SSE 代理、仅本机暴露数据服务、深度健康检查和离线部署模板。
...
| Agent 调度框架 | 95% | 默认 Agent 已通过门槛；MCP 服务化和 Dashboard trace 待扩展 |
| 数据/检索质量优化 | 进行中 | 诊断工具、自然问法 rewrite、治理候选已完成第一轮 |
...
```text
query
  -> controlled agent route / evidence policy
  -> auto keywords
...
聊天默认使用 `balanced` profile 和 `Qwen/Qwen3.5-27B`，Qwen3 交互问答关闭 thinking。API 默认启用受控 Agent，显式传入 `agent_mode=false` 可回退标准链路。
...
- 50 题人工关键词 balanced 检索诊断：`50/50`
- 50 题自然问法（忽略人工 keywords）balanced 检索诊断：`50/50`
- 自然问法 baseline vs Agent：Agent 改善 2、退化 0、持平 48
- 当前默认 Agent/Qwen 27B balanced 真实生成：`50/50` 成功、`50/50` 质量通过、0 降级
- 真实生成延迟：TTFT p50/p95 `0.668/5.053s`，总耗时 p50/p95 `22.876/31.578s`
- 安全/边界真实生成小样本：`8/8`
- 评估工具版本边界真实生成小样本：`5/5`
- 中文自然问法真实生成小样本：`5/5`
- 最新自然问法诊断：`data/retrieval_diagnostics/20260710_095333_balanced_natural_query`
- 最新自然问法对照：`data/qa_compare/20260710_092908_dry_run_compare`
- 最新真实生成全量基准：`data/qa_benchmarks/20260710_093128_cdba82.json`
...
| A5 对比评测 | 已完成第一版 | 支持人工关键词与 `--ignore-question-keywords` 自然问法对照 |
| API 接入 | 已完成 | `/ask`、`/ask/stream` 默认 Agent，支持 profile、状态和 timing |
...
全量自然问法 `evaluate_compare.py --ignore-question-keywords` 结果：

- 输出目录：`data/qa_compare/20260710_092908_dry_run_compare`
- total：50
- agent_improved：2
- agent_regressed：0
- agent_tied：48
- followup_triggered：2
- relation_delta.avg：0.48

解释：

去掉人工 keywords 后，deterministic query rewrite 已使 baseline 通过完整检索检查；Agent 只在诊断边界题触发补检索并增加关系证据。这说明：

- query rewrite 和证据组织是普遍召回质量的主要来源。
- Agent 在需要诊断边界补图谱关系时有局部收益，且没有引入退化。
- 后续应扩展更口语化、隐含意图更强的独立自然问法集，而不是继续围绕现有 50 题调参。
...
3. 评估覆盖风险
   当前 50 题已支持忽略人工关键词，但题目本身仍较结构化，不能完全覆盖真实用户口语问法。
